...
void CExperimentImgDlg::siftstitching() {
	...
	if (m_pImgSrc != NULL && m_pImgSrc != NULL) {
		...
		CString timeStr;
		timeStr.Format(_T("SIFT图像拼接耗时:%dms "), endTime - startTime);
		Output(timeStr);
	}
	else
...
void CExperimentImgDlg::stitching() {
	...
	if (m_pImgSrc != NULL && m_pImgSrc != NULL) {
		CImage* temp = new CImage();
		ImageProcess::stitch(name1, name2, temp);
		delete m_pImgResult;
		m_pImgResult = temp;

		clock_t endTime = clock();
		CString timeStr;
		timeStr.Format(_T("SURF图像拼接耗时:%dms "), endTime - startTime);
		Output(timeStr);
	}
	else
		AfxMessageBox(_T("缺少图片"));
...
void CExperimentImgDlg::detecting() {
	...
	ImageProcess::image_detection(cfgFile, weight, clsNames, image_path);//图片测试
	clock_t endTime = clock();
	CString timeStr;
	timeStr.Format(_T("yolo图像检测耗时:%dms "), endTime - startTime);
	Output(timeStr);
}
